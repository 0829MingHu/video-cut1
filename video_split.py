...
video_root='./duration10-30' #视频路径
thread_num=1 #线程数
every_duration=600 #每段时长，单位秒
...
class VideoClip:
    ...
    def split_video(self,source_video,every_duration,split_num):
        ...
        for i in range(split_num):
            ...
            os.system(cmd)
            start_time=self.get_time_str(every_duration,i)
    ...
    def split_all_format_file(self,mp4_file,every_duration):
        """
        ...
        #切割份数
        split_num=math.ceil(total_duration/every_duration)
        #切割份数少于2份，就不动了
        if split_num<2:
    ...
    def main(self,path):
        ...
        # for video in videos:
        #     self.split_all_format_file(video,every_duration)
        with ThreadPoolExecutor(max_workers=thread_num) as t:
            for video in videos:
                t.submit(self.split_all_format_file,video,every_duration)
